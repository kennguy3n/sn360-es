...
| **Ingestion poller** | No | Pull-based mailbox ingestion is disabled. No HTTP webhook path is mounted today; the push-notification handler (`POST /v1/push/{provider}/{tenant}`, see [ARCHITECTURE.md §5.1.2](./ARCHITECTURE.md#512-push-notification-receivers-implemented-not-yet-routed)) is unit-tested but not wired into `cmd/sn360-es/routes.go`, so a missing poller is effectively a global ingestion outage for affected tenants. | `ingestion_poller` check present if wired, never returns an error. | **Medium** — tenants that rely on poll-mode ingestion stop seeing new mail. |
