...
| **Redis** | No | URL-encrypt store, label-cache, and rate-limit token buckets fall back to in-memory equivalents. Banner / URL rewriting still produces tokens but they don't survive restart. | `redis` check absent; binary still ready. | **High** if banner tokens are mid-flight; user clicks may fail to validate after the restart that wipes the in-memory store. |
...
| **Ingestion poller** | No | Pull-based mailbox ingestion is disabled. Push-based webhook ingestion (POST /ingest) still works. | `ingestion_poller` check present if wired, never returns an error. | **Medium** — tenants that rely on poll-mode ingestion stop seeing new mail. |
