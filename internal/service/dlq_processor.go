...
// wildcards (es.evaluate.request[.>], es.evaluate.result[.>],
// es.action.>, es.onboarding.>, es.education.>). The list must stay
// in sync with the domains that the consumer-side defaultDLQSubject()
// helper in pkg/events/nats and pkg/events/redis can route to —
// otherwise dead-letter messages for the unlisted domain would land
// in the ES_DLQ stream but never be consumed, accumulating until
// age-based retention purges them.
