...
def project_one(profile: TrafficProfile, levers: CostLevers) -> Dict[str, float]:
    inf = cost_inference(profile, levers)
    cmp = cost_compute(profile, levers)
    pg = cost_postgres(profile, levers)
    s3 = cost_object_storage(profile, levers)
    rd = cost_redis(profile, levers)
    km = cost_kms(profile, levers)
    eg = cost_egress(profile, levers)
...
def project_all() -> List[Dict[str, float]]:
    out: List[Dict[str, float]] = []
    for profile in PROFILES.values():
        for levers in (CostLevers.baseline_off(), CostLevers.levers_on()):
            out.append(project_one(profile, levers))
    return out
...
def main() -> int:
    ...
    if args.profile == "all":
        ...
        records = [
            project_one(profile, CostLevers.baseline_off()),
            project_one(profile, CostLevers.levers_on()),
        ]
